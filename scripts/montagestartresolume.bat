...
if exist "C:\\Program Files\\Resolume Avenue\\Avenue.exe" (
	...
	start /b "" "C:\\Program Files\\Resolume Avenue\\Avenue.exe" > "%LOCALAPPDATA%\Montage\logs\resolume.log" 2>&1

) else if exist "C:\\Program Files\\Resolume Arena\\Arena.exe" (

	c:/windows/system32/taskkill /F /IM Arena.exe >nul 2>&1
	start /b "" "C:\\Program Files\\Resolume Arena\\Arena.exe" > "%LOCALAPPDATA%\Montage\logs\resolume.log" 2>&1
...
set osc="%MONTAGE%\bin\pyinstalled\osc.exe"
