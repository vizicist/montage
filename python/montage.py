...
Verbose = False
MyNuid = ""
MONTAGE_SOURCELogged = False

def localconfigFilePath(nm):
    return os.path.join(localAppDataDir(), "config", nm)

def configFilePath(nm):
    # If MONTAGE_SOURCE is defined, we use
    ps = os.environ.get("MONTAGE_SOURCE")
    if ps != None:
        global MONTAGE_SOURCELogged
        if not MONTAGE_SOURCELogged:
            MONTAGE_SOURCELogged = True
            print("Using MONTAGE_SOURCE=",ps," to get config files")
        return os.path.join(ps, "default", "config", nm)
    else:
        return os.path.join(MontageDir(), "config", nm)
...
def localAppDataDir():
    ...
    if local == None:
        ...
        local = "."
    return os.path.join(local,"Montage")

def presetsPath():
    p = ConfigValue("presetspath")
    p = p.replace("%MONTAGE%",MontageDir())
    lad = os.environ.get("LOCALAPPDATA")
    if lad != None:
        p = p.replace("%LOCALAPPDATA%",lad)

    # If MONTAGE_SOURCE is defined, add the default/presets directory
...
# which is either the user's LOCALAPPDATA version or (if MONTAGE_SOURCE is
# defined) the MONTAGE_SOURCE version.
...
def montage_api(meth, params=None):
    fullparams = "{ " + params + "}"
    r1,err = invoke_jsonrpc("montage.api",meth,fullparams)
    if err != None:
        print("API of ",meth," returned err=",err)
    return r1

def montage_publish(subject,params):
    ...
    try:
        ...
        print("montage_event: publish timed out, subject=%s params=%s\n" % (subject,params))
...
def MontageDir():
    global montageDir
    if montageDir == None:
        montageDir = os.environ.get("MONTAGE")
        if montageDir == None:
            print("MONTAGE environment variable needs to be defined.")
            exit()
    return montageDir
...
def SendCursorEvent(cid,ddu,x,y,z):
    ...
    e = ("{ \"nuid\": \"" + PythonNUID + "\", " + \
        ...
        "\"event\": \"" + event + "\", " + \
        "\"x\": \"%f\", \"y\": \"%f\", \"z\": \"%f\" }")  % (x,y,z)
    montage_publish("montage.event",e)
