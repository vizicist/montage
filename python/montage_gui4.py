...
def montage_region_api(region, meth, params=""):
    if region == "":
        print("montage_region_api: no region specified?  Assuming A")
        region = "A"
    if region == "*":
        print("montage_region_api: What do I do with a * region?")
        region = "A"
    p = "\"region\":\""+region+"\""
    if params == "":
        params = p
        ...
        params = p + "," + params
    return montage.montage_api("region."+meth,params)

def montage_global_api(meth, params=""):
    return montage.montage_api("global."+meth,params)
...
class ProGuiApp(tk.Tk):
    def __init__(self):
        ...
        self.winfo_toplevel().title("Montage")
    ...
    def readParamsFileIntoSnap(self,paramstype,paramsname):
        ...
        fpath = montage.presetsFilePath(paramstype, paramsname)
    ...
    def readSnapParamsFile(self,paramsname):
        ...
        if paramsname == "CurrentSnapshot":
            fpath = montage.localconfigFilePath(paramsname+".json")
        else:
            fpath = montage.presetsFilePath("snap", paramsname)
        print("Reading ",fpath)
        try:
            f = open(fpath)
        except:
    ...
    def makeStartupFrame(self,container):
        ...
        self.startupLabel = ttk.Label(f, text="               Montage is Loading...", style='Header.TLabel',
    ...
    def makeSelectorPage(self,parent,pagename,pagemaker):
        vals = montage.presetsListAll(pagename)
    ...
    def sendPadParamValue(self,pad,paramname,val):
        ...
        # else:
        montage_region_api(self.PadName,paramType+".set_param",
            "\"param\": \"" + paramname + "\"" + \
            ", \"value\": \"" + str(val) + "\"" )
    ...
    def savePrevious(self):
        ...
        montage.copyFile(frompath,topath)
    ...
    def sendPadPerformVal(self,pad,name):
        ...
        if name == "loopingonoff":
            ...
            montage_region_api(self.PadName, "loop_recording", '"onoff": "'+str(reconoff)+'"')
            montage_region_api(self.PadName, "loop_playing", '"onoff": "'+str(playonoff)+'"')

        elif name == "loopinglength":
            v = self.perpadPerformVal["loopinglength"][pad]["value"]
            montage_region_api(self.PadName, "loop_length", '"length": "'+str(v)+'"')

        elif name == "loopingfade":
            fade = self.perpadPerformVal["loopingfade"][pad]["value"]
            montage_region_api(self.PadName, "loop_fade", '"fadelength": "'+str(fade)+'"')

        elif name == "quant":
            val = self.perpadPerformVal["quant"][pad]["value"]
            montage_region_api(self.PadName, "set_param",
                "\"param\": \"" + "misc.quant" + "\"" + \
                ", \"value\": \"" + str(val) + "\"")
        elif name == "scale":
            ...
            val = 1.0
            montage_region_api(self.PadName, "loop_comb",
                "\"value\": \"" + str(val) + "\"")

        elif name == "midithru":
            thru = self.perpadPerformVal["midithru"][pad]["value"]
            montage_region_api(self.PadName, "midi_thru", "\"thru\": \"" + str(thru) + "\"")

        elif name == "useexternalscale":
            onoff = self.perpadPerformVal["useexternalscale"][pad]["value"]
            montage_region_api(self.PadName, "useexternalscale", "\"onoff\": \"" + str(onoff) + "\"")

        elif name == "midiquantized":
            quantized = self.perpadPerformVal["midiquantized"][pad]["value"]
            montage_region_api(self.PadName, "midi_quantized", "\"quantized\": \"" + str(quantized) + "\"")

        elif name == "transpose":
            val = self.globalPerformVal["transpose"][pad]["value"]
            montage_region_api(self.PadName, "set_transpose", "\"value\": \""+str(val) + "\"")

    def sendGlobalPerformVal(self,name):

        if name == "tempo":
            val = self.globalPerformVal["tempo"]["value"]
            montage_global_api("set_tempo_factor", "\"value\": \""+str(val) + "\"")

        # elif name == "configname":
        #     config = self.globalPerformVal["configname"]["value"]
        #     montage.setConfigName(config)
        #     print("CONFIGNAME setting to ",montage.getConfigName())

    def clearPadLoop(self,pad):
        montage_region_api(self.PadName, "loop_clear", "")
    ...
    def resetAll(self):
        ...
        montage_global_api("audio_reset")
    ...
    def clearExternalScale(self):
        montage_region_api(self.PadName, "clearexternalscale")

    def sendANO(self):
        montage_region_api(self.PadName, "ANO")
    ...
    def sendSnapPad(self,pad,paramstype=None):
        for pt in ["sound","visual","effect"]:
            ...
            if paramstype == None or paramstype == pt:
                montage_region_api(self.PadName, pt+".set_params", paramlistjson)
    ...
    def synthesizeParamsJson(self):
        ...
        s = ""
        path = montage.configFilePath("paramdefs.json")
        f = open(path,'r')
        lines = f.readlines() 
        for line in lines: 
            if "effect." in line:
    ...
    def readParamDefs(self):
        ...
        self.paramenums = montage.readJsonPath(montage.configFilePath("paramenums.json"))
        self.allEffectsJson = montage.readJsonPath(montage.configFilePath("resolume.json"))
        ...
        for pt in {"sound", "visual", "effect", "sliders"}:
            if pt in self.paramenums:
                ...
                self.paramenums[pt] = montage.presetsListAll(pt)

        j = montage.readJsonPath(montage.configFilePath("synths.json"))
    ...
    def convertParamdefsToParams(self,newparamsjson):
        ...
        # allparamsjson = montage.readJsonPath(montage.configFilePath("Params.json"))
    ...
    def sendPadOneEffectVal(self,pad,name,val):
        ...
        if i > 0:
            ...
            onoff = montage.boolValueOfString(val)
...
class PageEditParams(tk.Frame):
    ...
    def updateParamFiles(self):
        files = montage.presetsListAll(self.paramstype)
    ...
    def saveJson(self,section,paramsname,suffix=".json"):
        ...
        fpath = montage.localPresetsFilePath(section,paramsname,suffix)
    ...
    def normalizeJsonValue(self,name,v):
        t = self.controller.paramValueTypeOf[name]
        if t == "bool":
            return "true" if montage.boolValueOfString(v) else "false"
        if t == "int":
            v = int(v)
...
class PagePerformSliders(tk.Frame):
    ...
    def setSliders(self,slidersname):
        j = montage.readJsonPath(montage.presetsFilePath("sliders", slidersname))
...
def CurrentSnapshotPath():
    return montage.localconfigFilePath("CurrentSnapshot.json")

def CurrentSnapshotBackupPath():
    return montage.localconfigFilePath("CurrentSnapshot.backup")

def CurrentSnapshotPreviousPath():
    return montage.localconfigFilePath("CurrentSnapshot.previous")

def initMain(app):
    app.iconbitmap(montage.configFilePath("montage.ico"))
    app.mainLoop()

if __name__ == "__main__":

    gui_size = montage.ConfigValue("gui_size")
    if gui_size == "":
