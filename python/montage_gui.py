...
def montage_region_api(meth, params=""):
...
class ProGuiApp(tk.Tk):
    ...
    def sendPadParamValue(self,pad,paramname,val):
        ...
        # else:
        montage_region_api(paramType+".set_param",
            "\"param\": \"" + paramname + "\"" + \
            ", \"value\": \"" + str(val) + "\"" )
    ...
    def sendPadPerformVal(self,pad,name):
        ...
        if name == "loopingonoff":
            ...
            montage_region_api("loop_recording", '"onoff": "'+str(reconoff)+'"')
            montage_region_api("loop_playing", '"onoff": "'+str(playonoff)+'"')

        elif name == "loopinglength":
            v = self.perpadPerformVal["loopinglength"][pad]["value"]
            montage_region_api("loop_length", '"length": "'+str(v)+'"')

        elif name == "loopingfade":
            fade = self.perpadPerformVal["loopingfade"][pad]["value"]
            montage_region_api("loop_fade", '"fadelength": "'+str(fade)+'"')

        elif name == "quant":
            val = self.perpadPerformVal["quant"][pad]["value"]
            montage_region_api("set_param",
                "\"param\": \"" + "misc.quant" + "\"" + \
                ", \"value\": \"" + str(val) + "\"")
        elif name == "scale":
    ...
    def clearExternalScale(self):
        montage_region_api("clearexternalscale")

    def sendANO(self):
        montage_region_api("ANO")
    ...
    def sendSnapPad(self,pad,paramstype=None):
        for pt in ["sound","visual","effect"]:
            ...
            if paramstype == None or paramstype == pt:
                montage_region_api(pt+".set_params", paramlistjson)
